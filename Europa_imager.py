...
def Europa_imager():
    P_down = decibel(150, 1)                                                            #Power in dBW for downlink
    ...
    e_angle = 10*pi/180
    h = sqrt(1+dist_europa_sun*dist_europa_sun-2*dist_europa_sun*cos(e_angle))*AU - h_europa
    ...
    Sw = 2 * tan(sw_angle / 2) * h_europa
    V_g = sqrt(mu_europa / (Reuropa + h_europa)) * Reuropa / (Reuropa + h_europa)       #Ground speed in m/s
    ...
    R_up = decibel(1e3,1)                                                               #Data rate in dB(bit/s) for uplink
