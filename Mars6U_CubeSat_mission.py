...
def Mars6U_CubeSat():
    P_down = decibel(5, 1)                                                              #Power in dBW for downlink
    ...
    D_sc = 1                                                                            #Spacecraft antenna diameter in m
    ...
    D_gr = 10                                                                           #Ground antenna diameter in m
    ...
    h_mars = 500 * 1000                                                                 #Orbit altitude in m
    e_angle = 10*pi/180                                                                 #Elongation angle
    h = sqrt(1*1+dist_mars_sun*dist_mars_sun-2*dist_mars_sun*cos(e_angle))*AU- h_mars
    ...
    payload_pix_size = float(0.2/60) * pi / 180                                         #Payload pixel size in rad
    payload_bits_per_pix = float(8)                                                     #Payload bits per pixel
    pixel_line_depth = 2 * tan(payload_pix_size / 2) * h_mars                           #Pixel line depth
    Sw = 2 * tan(sw_angle / 2) * h_mars
    V_g = sqrt(mu_mars / (Rmars + h_mars)) * Rmars / (Rmars + h_mars)                   #Ground speed in m/s
    D_r = Sw * V_g * payload_bits_per_pix/ (pixel_line_depth*pixel_line_depth)          #Generated data rate
    D_c = 100 / 100                                                                     #Spacecraft duty cycle
    T_down = 12 / 24                                                                    #Spacecraft downlink time in sec
    R_down = decibel(D_r * D_c / T_down, 1)                                             #Data rate in dB(bit/s) for downlink
    R_up = decibel(1e5,1)                                                               #Data rate in dB(bit/s) for uplink

    k_ = decibel(float(k), 1)                                                           #Bolztmann constant in dB(J/K)

    L_i = 0                                                                             #Implementation losses in dB

    return [P_down, L_l, G_t_down, L_a, G_r_down, L_s_down, L_pr_down_sc+L_pr_down_gr, L_r, -T_s_down, -R_down, -k_, -L_i], \
